...
// Helper function to check if cache entry exists and has data
template<typename CacheType>
bool cache_entry_exists(const CacheType& cache, int index1, int index2) {
    if (index1 < cache.get_max_nindex() || index1 > cache.get_max_pindex()) return false;
    if (index2 < cache[index1].get_max_nindex() || index2 > cache[index1].get_max_pindex()) return false;
...
class FK {
        ...
        void f (const std::vector<int>& angles) {
            ...
            for (int ind = 0; ind < crossings; ind++) {
                if (r[ind] == 1) {  // Crossing type 1
                    ...
                    if (!cache_entry_exists(binom_cache, i, i-m)) {
                         throw std::runtime_error("Quantum Binomial cache entry [" + std::to_string(i) + "][" + std::to_string(i-m) +
                                                "] not found. Please expand cache by running caching_knots with higher degree.");
                    }
                    term = cache_polynomial_multiply(binom_cache[i][i-m], term, X_MAX[0]);  // Quantum Binomial 
                }
                else if (r[ind] == 2) {  // Crossing type 2
                    int i = numerical_assignment[matrices[ind][0][0]][matrices[ind][0][1]];  // i
                    int m = numerical_assignment[matrices[ind][3][0]][matrices[ind][3][1]];  // j' (m)
                    if (!cache_entry_exists(binom_cache, i, m)) {
                        throw std::runtime_error("Quantum Binomial cache entry [" + std::to_string(i) + "][" + std::to_string(m) +
                                                 "] not found. Please expand cache by running caching_knots with higher degree.");
                     }
                     term = cache_polynomial_multiply(binom_cache[i][m], term, X_MAX[0]);  // Quantum Binomial 
                }
                else if (r[ind] == 3) {  // Crossing type 3    
                    int j = numerical_assignment[matrices[ind][1][0]][matrices[ind][1][1]];  // j
                    int k = numerical_assignment[matrices[ind][2][0]][matrices[ind][2][1]];  // i' (k)
                    if (!cache_entry_exists(binom_cache, j, k)) {
                        throw std::runtime_error("Quantum Binomial cache entry [" + std::to_string(j) + "][" + std::to_string(k) +
                                                  "] not found. Please expand cache by running caching_knots with higher degree.");
                    }
                    term = cache_polynomial_multiply_reversed(binom_cache[j][k], term, X_MAX[0]);  // Quantum Binomial
                }
                else {  // Crossing type 4
                    int j = numerical_assignment[matrices[ind][1][0]][matrices[ind][1][1]];  // j
                    int k = numerical_assignment[matrices[ind][2][0]][matrices[ind][2][1]];  // i' (k)
                    if (!cache_entry_exists(binom_cache, j, j-k)) {
                        throw std::runtime_error("Quantum Binomial cache entry [" + std::to_string(j) + "][" + std::to_string(j-k) +
                                                   "] not found. Please expand cache by running caching_knots with higher degree.");
                    }
                    term = cache_polynomial_multiply_reversed(binom_cache[j][j-k], term, X_MAX[0]);  // Quantum Binomial
                }
            }
            ...
            for (int ind = 0; ind < crossings; ind++) {
                if (r[ind] == 1) {  // Crossing type 1
                    ...
                    int j = numerical_assignment[matrices[ind][1][0]][matrices[ind][1][1]];  // j
                    int k = numerical_assignment[matrices[ind][2][0]][matrices[ind][2][1]];  // i' (k)
                    if (!cache_entry_exists(poch_cache, k, j + 1)) {
                        throw std::runtime_error("Pochhammer cache entry [" + std::to_string(k) + "][" + std::to_string(j + 1) + 
                                               "] not found. Please expand cache by running caching_knots with higher degree.");
                    }
                    term = cache_polynomial_multiply(poch_cache[k][j + 1], term, X_MAX[b]);  // (x_b*q)_k / (x_b*q)_{j+1}
                }
                else if (r[ind] == 2) {  // Crossing type 2
                    int b = bottom_crossing_components[ind];  // Bottom component
                    int j = numerical_assignment[matrices[ind][1][0]][matrices[ind][1][1]];  // j
                    ...
                    if (!cache_entry_exists(inv_poch_cache, j, k + 1)) {
                        throw std::runtime_error("Inverse Pochhammer cache entry [" + std::to_string(j) + "][" + std::to_string(k + 1) + 
                                               "] not found. Please expand cache by running caching_knots with higher degree.");
                    }
                    term = cache_polynomial_multiply(inv_poch_cache[j][k + 1], term, X_MAX[b]);  // Inverse Pochhammer
                }
                else if (r[ind] == 3) {  // Crossing type 3
                    ...
                    if (!cache_entry_exists(inv_poch_cache, i, m + 1)) {
                        throw std::runtime_error("Inverse Pochhammer cache entry [" + std::to_string(i) + "][" + std::to_string(m + 1) + 
                                               "] not found. Please expand cache by running caching_knots with higher degree.");
                    }
                    term = cache_polynomial_multiply(inv_poch_cache[i][m + 1], term, X_MAX[t]);  // Inverse Pochhammer
                }
                else {  // Crossing type 4
                    int t = top_crossing_components[ind];  // Top component
                    int i = numerical_assignment[matrices[ind][0][0]][matrices[ind][0][1]];  // i
                    int m = numerical_assignment[matrices[ind][3][0]][matrices[ind][3][1]];  // j' (m)
                    if (!cache_entry_exists(poch_cache, m, i + 1)) {
                        throw std::runtime_error("Pochhammer cache entry [" + std::to_string(m) + "][" + std::to_string(i + 1) + 
                                               "] not found. Please expand cache by running caching_knots with higher degree.");
                    }
                    term = cache_polynomial_multiply(poch_cache[m][i + 1], term, X_MAX[t]);  // (x_t*q)_m / (x_t*q)_{i+1}
                }
            } 
    ...
    public:
        ...
        bilvector<bilvector<std::vector<std::vector<int>>>> poch_cache;  ///< Cached q-Pochhammer values for positive indices
        bilvector<bilvector<std::vector<std::vector<int>>>> inv_poch_cache;  ///< Cached inverse q-Pochhammer values for negative indices
        bilvector<bilvector<std::vector<std::vector<int>>>> binom_cache;  ///< Cached q-binomial coefficient values
        /**
        ...
        FK (std::string infile_, std::string outfile_) {
            ...
            */
            ...
            // Read cached q-Pochhammer values from file
            poch_cache = read_pochhammer_cache("Data/Caches/caching_knots_pochhammers.txt", degree + 1);
            
            inv_poch_cache = read_pochhammer_cache("Data/Caches/caching_knots_inv_pochhammers.txt", degree + 1);
            
            binom_cache = read_pochhammer_cache("Data/Caches/caching_knots_binomials.txt", 1);
