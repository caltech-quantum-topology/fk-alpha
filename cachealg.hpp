...
/**
 * @brief Multiplies cached polynomial data with vector of bilvectors
 * 
 * @param cache_poly Vector of vectors representing cached polynomial data
 *                   Each row format: [min_degree, max_degree, coeff_min, coeff_min+1, ..., coeff_max]
 * @param input_poly Vector of bilvectors representing input polynomial (power series with Laurent coefficients)
 * @param X_MAX Maximum power series degree to compute in output
 * @return Vector of bilvectors representing the product polynomial
 * 
 * The multiplication algorithm:
 * - For each term in cache_poly (up to degree X_MAX) and each term in input_poly
 * - Multiply coefficients and add exponents
 * - Only keep results where power series degree <= X_MAX
 */
inline std::vector<bilvector<int64_t>> cache_polynomial_multiply(
        ...
        for (int cache_deg = 0; cache_deg <= max_cache_deg; cache_deg++) {
            ...
            for (int cache_laurent_idx = 0; cache_laurent_idx < expected_coeffs; cache_laurent_idx++) {
                ...
                for (int input_laurent_deg = input_laurent.get_max_nindex(); 
                     input_laurent_deg <= input_laurent.get_max_pindex(); 
                     input_laurent_deg++) {
...
inline std::vector<bilvector<int64_t>> cache_polynomial_multiply_reversed(
    const std::vector<std::vector<int>>& cache_poly,
    const std::vector<bilvector<int64_t>>& input_poly,
    int X_MAX
) {
