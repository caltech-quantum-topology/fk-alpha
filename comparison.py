...
def main(title, braid, degree):
    print(f"Processing knot {title} with braid {braid} and degree {degree}")
    assignment = get_sign_assignment(braid, degree=10, verbose=False)
    ...
    os.system(f'./a.out "Data/Input/{title}" "Data/Output/{title}a"')
    os.system(f'./b.out "Data/Input/{title}" "Data/Output/{title}b"')

    if not os.path.exists(f'Data/Output/{title}a.json'):
        raise Exception(f"Output file for knot {title} (a) not found after C++ computation")

    if not os.path.exists(f'Data/Output/{title}b.json'):
         raise Exception(f"Output file for knot {title} (b) not found after C++ computation")

    # check that the two outputs agree
    import json

    with open(f'Data/Output/{title}a.json', 'r') as f:
        output_a = json.load(f)

    with open(f'Data/Output/{title}b.json', 'r') as f:
        output_b = json.load(f)

    if output_a == output_b:
        print(f"✓ JSON structures for {title} match perfectly")
        return True
    else:
        print(f"✗ JSON structures for {title} differ")
        print(f"Output A keys: {list(output_a.keys()) if isinstance(output_a, dict) else type(output_a)}")
        print(f"Output B keys: {list(output_b.keys()) if isinstance(output_b, dict) else type(output_b)}")
        return False
